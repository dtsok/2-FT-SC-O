...
{
	if (running) {
		return (getElapsedTime() + base_time);
	}
	else {
		return base_time;
	}
...
void RFWTimer::setMaxTime(double mt)
{
	max_time = mt;
}
double RFWTimer::getMaxTime()
{
	return max_time;
}
...
double RFWTimer::getTimeToExpire()
{ // may be negative!
	if (getMaxTime() == 0) {
...
void RFWTimer::setBaseTime(double bt)
{
	base_time = bt;
}
...
double RFWTimer::getUserTime()
{
	...
	if (msecs > 0) {
		...
		return 0.0; // sometimes msecs is -0.000 (go figure...)
	}
}
